...
class CodeRevisionView extends CodeView {
	...
	protected function tagForm() {
		...
		if ( $this->performer->isAllowed( 'codereview-add-tag' ) ) {
			$list .= self::addTagForm( $this->mAddTags, $this->mRemoveTags );
		}
		return $list;
	}
	...
	protected function commentLink( $commentId ) {
		$repo = $this->mRepo->getName();
		$rev = $this->mRev->getId();
		return SpecialPage::getTitleFor( 'Code', "$repo/$rev", "c{$commentId}" );
	}
	...
	protected function revLink() {
		$repo = $this->mRepo->getName();
		$rev = $this->mRev->getId();
		return SpecialPage::getTitleFor( 'Code', "$repo/$rev" );
	}
